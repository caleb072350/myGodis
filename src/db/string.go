...
func Incr(db *DB, args [][]byte) redis.Reply {
	...
	if bytes != nil {
		...
		db.Data.Put(key, &DataEntity{Data: []byte(strconv.FormatInt(val+1, 10))})
		return reply.MakeIntReply(val + 1)
	} else {
		db.Data.Put(key, &DataEntity{Data: []byte("1")})
...
func IncrBy(db *DB, args [][]byte) redis.Reply {
	...
	if bytes != nil {
		...
		db.Data.Put(key, &DataEntity{Data: []byte(strconv.FormatInt(val+delta, 10))})
		return reply.MakeIntReply(val + delta)
	} else {
		db.Data.Put(key, &DataEntity{Data: args[1]})
...
func IncrByFloat(db *DB, args [][]byte) redis.Reply {
	...
	if bytes != nil {
		...
		db.Data.Put(key, &DataEntity{Data: resultBytes})
